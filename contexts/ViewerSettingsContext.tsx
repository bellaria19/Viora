...
export const ViewerSettingsProvider: React.FC<ViewerSettingsProviderProps> = ({ children }) => {
  // 각 뷰어 설정 상태
  const [textViewerOptions, setTextViewerOptions] = useState<TextViewerOptions>(defaultTextViewerOptions);
  const [pdfViewerOptions, setPDFViewerOptions] = useState<PDFViewerOptions>(defaultPDFViewerOptions);
  const [imageViewerOptions, setImageViewerOptions] = useState<ImageViewerOptions>(defaultImageViewerOptions);
  const [epubViewerOptions, setEPUBViewerOptions] = useState<EPUBViewerOptions>(defaultEPUBViewerOptions);
  const [zipViewerOptions, setZipViewerOptions] = useState<ZipImageViewerOptions>(defaultZipViewerOptions);
  ...
  return <ViewerSettingsContext.Provider value={contextValue}>{children}</ViewerSettingsContext.Provider>;
};
