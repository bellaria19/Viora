...
import { DuplicateFile, pickFiles, checkDuplicates, processFiles } from '@/utils/filePickerManager';
...
export function useFilePicker({ existingFiles, onFilesProcessed }: UseFilePickerProps) {
  ...
  const currentDuplicateFile = currentDuplicateIndex >= 0 ? duplicateFiles[currentDuplicateIndex] : null;
